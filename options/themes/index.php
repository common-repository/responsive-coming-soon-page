<?php
// Start session safely if needed
if (!session_id()) {
	session_start();
}

// Get options with default fallback
$wl_rcsm_options = get_option('weblizar_rcsm_options', array());
...
<head>
	<title><?php echo esc_html($wl_rcsm_options['page_title'] ?? ''); ?></title>
	<meta charset="utf-8">
	...
	<?php if (!empty($wl_rcsm_options['search_robots']) && $wl_rcsm_options['search_robots'] === 'on') { ?>
		<meta name="robots" content="<?php echo esc_attr($wl_rcsm_options['rcsm_robots_meta'] ?? ''); ?>">
	<?php } ?>
	<?php
	$favicon_img = !empty($wl_rcsm_options['upload_favicon']) ? esc_url($wl_rcsm_options['upload_favicon']) : esc_url(RCSM_PLUGIN_URL . 'options/images/favicon.png');
	?>
	<link rel="icon" href="<?php echo esc_url($favicon_img); ?>">
	<meta name="author" content="<?php echo esc_attr($wl_rcsm_options['page_author'] ?? ''); ?>">

	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js" type="text/javascript"></script>
	<script src="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/themes/js/scrollReveal.min.js'); ?>"></script>
	<script src="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/themes/js/script-frontend.js'); ?>"></script>
	<link rel="stylesheet" href="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/css/all.min.css'); ?>">
	<link rel="stylesheet" href="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/themes/css/animate.min.css'); ?>">
	<link rel="stylesheet" href="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/css/bootstrap.min.css'); ?>">
	<link rel="stylesheet" href="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/themes/css/custom.css'); ?>">
	<link rel="stylesheet" href="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/themes/css/media-screen.css'); ?>">
	<script src="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/themes/js/countdown.js'); ?>"></script>
	<script src="<?php echo esc_url(RCSM_PLUGIN_URL . 'options/js/bootstrap.min.js'); ?>"></script>
	...
	<?php
	if (!empty($wl_rcsm_options['google_analytics'])) {
		echo '<script>' . esc_js($wl_rcsm_options['google_analytics']) . '</script>';
	}
	if (!empty($wl_rcsm_options['custom_css'])) {
		echo '<style>' . esc_html(wp_strip_all_tags($wl_rcsm_options['custom_css'])) . '</style>';
	}
	?>
...
<body>
	<div id="wrapper">
			...
			<header>
				<div class="social-icons">
					<ul class="animated fadeInDownBig">
						<?php
						for ($i = 1; $i <= 5; $i++) {
							if (!empty($wl_rcsm_options['social_icon_' . $i])) { ?>
								<li><a target="<?php echo !empty($wl_rcsm_options['link_tab_' . $i]) && $wl_rcsm_options['link_tab_' . $i] == 'on' ? '_blank' : '_self'; ?>" href="<?php echo esc_url($wl_rcsm_options['social_link_' . $i]); ?>"><i class="<?php echo esc_attr($wl_rcsm_options['social_icon_' . $i] . ' icon'); ?>"></i></a></li>
						<?php }
						} ?>
					</ul>
				</div>
			...
			<div class="container">
				<div class="carousel-caption form_align">
					<h4 class="logo_class ff">
						<?php
						$site_logo_value = $wl_rcsm_options['site_logo'] ?? '';
						if ($site_logo_value == 'logo_text') {
							echo esc_html($wl_rcsm_options['logo_text_value'] ?? '');
						} elseif (empty($wl_rcsm_options['upload_image_logo'])) {
							echo esc_html($wl_rcsm_options['logo_text_value'] ?? '');
						} else { ?>
					...
					<?php
					$layout_status = $wl_rcsm_options['layout_status'] ?? '';
					...
					if ($button_onoff == 'on' && !empty($wl_rcsm_options['button_text'])) { ?>
		...
		$data = $wl_rcsm_options['page_layout_swap'] ?? array();
		if (!empty($data)) {
		...
		<div class="container-fluid copyright">
			<div class="social-icons">
				<ul class="social animated fadeInDownBig">
					...
					for ($i = 1; $i <= 5; $i++) {
						if (!empty($wl_rcsm_options['social_icon_' . $i])) { ?>
							<li><a target="<?php echo !empty($wl_rcsm_options['link_tab_' . $i]) && $wl_rcsm_options['link_tab_' . $i] == 'on' ? '_blank' : '_self'; ?>" href="<?php echo esc_url($wl_rcsm_options['social_link_' . $i]); ?>"><i class="<?php echo esc_html($wl_rcsm_options['social_icon_' . $i]); ?> icon"></i></a></li>
					<?php }
					} ?>
				</ul>
			</div>
		...
		<?php
		if (!empty($wl_rcsm_options['link_admin']) && $wl_rcsm_options['link_admin'] === 'on' && is_user_logged_in()) {
			global $current_user;
			wp_get_current_user();
			$LoggedInUserID = $current_user->ID;
			$UserData = get_userdata($LoggedInUserID);
			if (isset($UserData->roles[0]) && $UserData->roles[0] === 'administrator' && !empty($wl_rcsm_options['admin_link_text'])) { ?>
				<a class="btn left_side_link" href="<?php echo esc_url(get_admin_url() . "admin.php?page=rcsm-weblizar"); ?>" data-sr="enter bottom over 1s and move 110px wait 0.3s"> <?php echo esc_html($wl_rcsm_options['admin_link_text']); ?></a>
		<?php }
		} ?>
